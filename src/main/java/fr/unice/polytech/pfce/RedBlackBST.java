...
public class RedBlackBST<Key extends Comparable<Key>> implements BinaryTree<Key> {
    ...
    private Node fromSortedArray(Key[] arr, int start, int end) {
        ...
        int mid = (start+end) / 2;
        Node node = new Node(arr[mid], BLACK, 1);
    ...
    private Node remove(Node h, Key key) {
        ...
        if (key.compareTo(h.key) < 0)  {
            ...
            if (key.compareTo(h.key) == 0) {
                ...
                h.key = x.key;
                // h.val = get(h.right, findMin(h.right).key);
    ...
    // is the tree rooted at x a BST with all keys strictly between findMin and findMax
    // (if findMin or findMax is null, treat as empty constraint)
    ...
    private boolean isBalanced() {
        int black = 0;     // number of black links on path from root to findMin
        Node x = root;
        while (x != null) {
            if (!isRed(x)) black++;
            x = x.left;
